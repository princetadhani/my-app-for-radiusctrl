...
import { Zap, Network, FileText, Sparkles, ChevronRight, Command, Radio } from 'lucide-react';
...
export function CoaEmptyState() {
  ...
  const orbIcons = [
    { Icon: Zap, label: 'COA Requests', color: '#7aa2f7', gradient: 'from-blue-500 to-cyan-500' },
    { Icon: Radio, label: 'Disconnect', color: '#f7768e', gradient: 'from-red-500 to-pink-500' },
    { Icon: Network, label: 'NAS Devices', color: '#9ece6a', gradient: 'from-green-500 to-emerald-500' },
    { Icon: FileText, label: 'Templates', color: '#bb9af7', gradient: 'from-purple-500 to-violet-500' },
  ];
    ...
    >
      ...
      <div className="absolute inset-0 opacity-30">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <linearGradient id="grad1-coa" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <pattern id="dots-coa" x="0" y="0" width="50" height="50" patternUnits="userSpaceOnUse">
              <circle cx="2" cy="2" r="1" fill="#7aa2f7" opacity="0.3" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#dots-coa)" />
        </svg>
      </div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-16 w-full">
          ...
          >
            ...
            <p className="text-lg sm:text-xl text-gray-400 mb-8 leading-relaxed">
              Select a COA template from the <span className="text-blue-400 font-semibold">sidebar</span> to send Change-of-Authorization or Disconnect requests to your NAS devices.
            </p>

            {/* Quick Actions Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full mb-8">
              {[
                { icon: Zap, label: 'Execute COA', desc: 'Send COA requests', color: '#7aa2f7' },
                { icon: Command, label: 'Quick Open', desc: 'Ctrl/Cmd + K', color: '#bb9af7' },
                { icon: Radio, label: 'Disconnect Users', desc: 'Force client disconnect', color: '#f7768e' },
                { icon: FileText, label: 'Templates', desc: 'Create & manage files', color: '#9ece6a' },
              ].map((action, index) => (
                <motion.div
                  key={index}
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-400 text-xs">for quick template search</span>
              </div>
            </motion.div>
          </motion.div>
