...
import { freeradius, systemctl } from '../utils/hostCommand';
  ...
  try {
    ...
    try {
      ...
      const errorLines = output.split('\n').filter(line =>
        ...
        line.includes('Entry') ||
        line.includes('begin') ||
        line.includes('Entry does not begin with a user name') ||
        line.includes('Could not open included file') ||
        line.includes('included')
      ).slice(0, 15);
